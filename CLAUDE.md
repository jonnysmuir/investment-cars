...
- **Per-model body type scraping**: For makes in `MAKE_LEVEL_BODY_SPLITS`, `scrapeMake` runs per-model + per-body-type searches instead of broad make-level searches. This avoids pagination limits — e.g. searching "BMW Coupe" returns all BMW coupes across all models, but searching "BMW M4 Coupe" returns only M4 coupes. `getRelevantBodyTypes()` uses `bodyTypeRules` to skip irrelevant body types (e.g. M2 only searches Coupe, not Convertible/Saloon/Estate). Context is rotated every 5 models with 10-20s delays between models.
